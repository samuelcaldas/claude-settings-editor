(function exposeI18n(root, factory) {
  ...
  const DICTIONARIES = {
    en: {
      ...
      'sections.models.tiers.title': 'Gateway Model Tiers & Family Aliases (ANTHROPIC_DEFAULT_*)',
      'sections.models.tiers.desc': 'Configure family-level default model mappings and aliases (stored in ANTHROPIC_DEFAULT_*) to route entire model families (fable, opus, sonnet, haiku) through gateway endpoints.',
      ...
      'setting.env_CLAUDE_CODE_DISABLE_UNKNOWN_MODEL_WINDOW_ENFORCEMENT.label': 'Skip Compaction for Unknown Models',
      'setting.env_CLAUDE_CODE_ATTRIBUTION_HEADER.label': 'System Prompt Attribution Header',
      'setting.env_CLAUDE_STREAM_FIRST_BYTE_TIMEOUT_MS.label': 'Stream First Byte Timeout (ms)',
      ...
      'env.selectVar.label': 'Select Claude CLI Variable',
      'env.selectVar.placeholder': 'Choose a reviewed variable',
      'env.search': 'Search names and descriptions',
      'env.category': 'Category',
      'env.category.all': 'All categories',
      'env.category.behavior': 'Behavior',
      'env.category.tools': 'Tools and shell',
      'env.category.network': 'Networking and providers',
      'env.category.telemetry': 'Telemetry and diagnostics',
      'env.category.performance': 'Performance and limits',
      'env.category.managed': 'Managed & enterprise policies',
      'env.bool.on': '1 (on)',
      'env.bool.off': '0 (off)',
      'env.bool.true': 'true (on)',
      'env.bool.false': 'false (off)',
      'env.bool.toggle': 'Toggle boolean value',
      'env.includeUnofficial': 'Include useful unofficial variables',
      'env.discoveryHint': 'A reviewed selection, not a complete list. Custom variables remain editable.',
      'env.results': '{count} suggestions',
      'env.noResults': 'No suggestions match. Clear the filters or enter a custom name below.',
      'env.name': 'Variable name',
      'env.value': 'String value',
      'env.emptyHint': 'Empty stores an empty string; removing the variable is a separate action. Restart Claude Code to apply removals.',
      'env.mask.show': 'Show values',
      'env.mask.hide': 'Hide values',
      'env.editDedicated': 'Edit existing field',
      'env.editRaw': 'Edit in Advanced JSON',
      'env.row.key': 'Variable name: {name}',
      'env.row.value': 'Value for {name}',
      'env.error.name': 'Use an uppercase variable name: letters, digits, and underscores; do not start with a digit.',
      'env.error.duplicate': 'That variable already exists. Edit its existing row instead; your draft has been retained.',
      'env.error.dedicated': 'This variable has a dedicated field. Use “Edit existing field” in the help above.',
      'env.help.label': 'Variable details',
      'env.help.custom': 'Custom variable. Its runtime behavior is not verified by this editor; schema validation still applies.',
      'env.help.sourceDescription': 'Source-based description (English)',
      'env.help.source': 'Description source',
      'env.help.provenance': 'Variable provenance',
      'env.help.observed': 'Observed in unofficial CLI v{version} reference; current support is unverified.',
      'env.help.minimum': 'Documented minimum CLI version: {version}. Installed version is not detected.',
      'env.help.restart': 'Read during startup: relaunch Claude Code after changing this value.',
      'env.help.ignoredScope': 'Ignored in this target scope. Use the launch environment or a supported settings scope.',
      'env.help.sensitive': 'May contain credentials or expose sensitive diagnostic content. Masking is not encryption; exports retain values.',
      'env.help.values': 'String-value guidance: {values}. Guidance does not replace schema validation.',
      'env.help.multiline': 'Preserved value; edit multiline or non-string data in Advanced JSON.',
      'env.status.official': 'Officially documented',
      'env.status.unofficial': 'Unofficial — opt-in reference',
      'env.status.unclassified': 'Unclassified reference — not a verified recommendation',
      'env.applicability.settings': 'Configurable through settings, subject to scope and version restrictions.',
      'env.applicability.subprocess': 'In settings.json, this affects subprocesses only, not Claude’s interface.',
      'env.applicability.launch': 'Launch environment only; ignored in settings.json.',
      'env.applicability.ignored': 'Runtime-owned; ignored in settings.json.',
      'env.applicability.unknown': 'Settings applicability and load timing have not been verified.',
      'env.guidance.nonempty': 'Any nonempty value, including “0” and “false”, enables disabling. Do not treat “0” as off.',
      'env.guidance.afk': 'Auto-continue is off by default; “0” closes immediately. The 60-second default applied only in v2.1.198–199.',
      'env.guidance.pwsh': 'Parser timeout, not command execution timeout. Current behavior is unverified.',
      'env.guidance.slowOperation': 'Diagnostic logging can expose sensitive content. No logging is enabled automatically.',
      'env.guidance.cacheCost': 'One-hour cache writes cost more than five-minute cache writes.',
      ...
      'models.discovery.status.noCreds': 'API URL and Key required for model discovery.',
      'models.discovery.status.empty': 'No API URL and key configured. Models list is empty.',
      'models.discovery.badge.defaults': '{count} defaults',
      'models.discovery.badge.loaded': '{count} models',
      'models.discovery.badge.empty': '0 models',
      'models.discovery.hint': 'Queries the OpenAI-compatible /v1/models endpoint to populate model dropdowns.',
      'models.ai.generateBtn': 'AI',
      'models.ai.generateTitle': 'Generate description with AI using configured API',
      'models.ai.generating': 'Generating description...',
      'models.ai.success': 'Model description generated successfully.',
      'models.ai.error': 'Failed to generate description: {error}',
      'models.ai.noModel': 'Enter a Model ID before generating a description.',
      'models.ai.noApi': 'API URL and Key required to generate description.',
      ...
      'models.field.streamFirstByteTimeout': 'First Byte Stream Timeout (ms)',
      'models.placeholder.streamFirstByteTimeout': 'e.g. 60000',
      'models.hint.streamFirstByteTimeout': 'Deadline in ms for the first response byte of a streaming request (e.g. 60000 for gateways).',
      'models.tiers.presets': 'Quick Presets:',
      'models.tiers.presetAlanwo': '+ Alanwo Gateway',
      ...
      'toggle.disableUnknownModelWindowEnforcement': 'Skip auto-compaction for unknown gateway model IDs',
      'toggle.attributionHeaderZero': 'Omit system prompt attribution header (gateway caching)',
      ...
      'models.overrides.presetGateway': '+ Gateway Defaults',
    ...
    'pt-BR': {
      ...
      'sections.models.tiers.title': 'Níveis de Modelo do Gateway e Aliases de Família (ANTHROPIC_DEFAULT_*)',
      'sections.models.tiers.desc': 'Configure mapeamentos e aliases padrão em nível de família (em ANTHROPIC_DEFAULT_*) para rotear famílias inteiras de modelos (fable, opus, sonnet, haiku) via gateway.',
      ...
      'setting.env_CLAUDE_CODE_DISABLE_UNKNOWN_MODEL_WINDOW_ENFORCEMENT.label': 'Ignorar Compactação para Modelos Desconhecidos',
      'setting.env_CLAUDE_CODE_ATTRIBUTION_HEADER.label': 'Cabeçalho de Atribuição do Prompt do Sistema',
      'setting.env_CLAUDE_STREAM_FIRST_BYTE_TIMEOUT_MS.label': 'Tempo Limite do Primeiro Byte de Streaming (ms)',
      ...
      'env.selectVar.label': 'Selecionar Variável do Claude CLI',
      'env.selectVar.placeholder': 'Escolha uma variável revisada',
      'env.search': 'Pesquisar nomes e descrições',
      'env.category': 'Categoria',
      'env.category.all': 'Todas as categorias',
      'env.category.behavior': 'Comportamento',
      'env.category.tools': 'Ferramentas e shell',
      'env.category.network': 'Rede e provedores',
      'env.category.telemetry': 'Telemetria e diagnósticos',
      'env.category.performance': 'Desempenho e limites',
      'env.category.managed': 'Políticas gerenciadas e corporativas',
      'env.bool.on': '1 (ligado)',
      'env.bool.off': '0 (desligado)',
      'env.bool.true': 'true (ligado)',
      'env.bool.false': 'false (desligado)',
      'env.bool.toggle': 'Alternar valor booleano',
      'env.includeUnofficial': 'Incluir variáveis não oficiais úteis',
      'env.discoveryHint': 'Seleção revisada, não uma lista exaustiva. Variáveis personalizadas continuam editáveis.',
      'env.results': '{count} sugestões',
      'env.noResults': 'Nenhuma sugestão encontrada. Limpe os filtros ou informe um nome personalizado abaixo.',
      'env.name': 'Nome da variável',
      'env.value': 'Valor em texto',
      'env.emptyHint': 'Deixar vazio grava uma string vazia; remover a variável é uma ação separada. Reinicie o Claude Code para aplicar remoções.',
      'env.mask.show': 'Exibir valores',
      'env.mask.hide': 'Ocultar valores',
      'env.editDedicated': 'Editar campo existente',
      'env.editRaw': 'Editar no JSON Avançado',
      'env.row.key': 'Nome da variável: {name}',
      'env.row.value': 'Valor para {name}',
      'env.error.name': 'Use um nome em maiúsculas: letras, dígitos e sublinhados; não inicie com dígito.',
      'env.error.duplicate': 'Essa variável já existe. Edite a linha existente; seu rascunho foi mantido.',
      'env.error.dedicated': 'Esta variável possui um campo dedicado. Use “Editar campo existente” nas instruções acima.',
      'env.help.label': 'Detalhes da variável',
      'env.help.custom': 'Variável personalizada. O comportamento em execução não é verificado por este editor; a validação do schema continua ativa.',
      'env.help.sourceDescription': 'Descrição da fonte (em inglês)',
      'env.help.source': 'Fonte da descrição',
      'env.help.provenance': 'Procedência da variável',
      'env.help.observed': 'Observada na referência não oficial da CLI v{version}; suporte atual não verificado.',
      'env.help.minimum': 'Versão mínima documentada da CLI: {version}. A versão instalada não é detectada.',
      'env.help.restart': 'Lida na inicialização: reinicie o Claude Code após alterar este valor.',
      'env.help.ignoredScope': 'Ignorada neste escopo de destino. Use o ambiente de inicialização ou um escopo suportado.',
      'env.help.sensitive': 'Pode conter credenciais ou expor diagnósticos sensíveis. A máscara não é criptografia; exportações mantêm o valor.',
      'env.help.values': 'Orientações de valor em texto: {values}. A orientação não substitui a validação do schema.',
      'env.help.multiline': 'Valor preservado; edite valores com quebra de linha ou não-texto no JSON Avançado.',
      'env.status.official': 'Oficialmente documentada',
      'env.status.unofficial': 'Não oficial — referência opcional',
      'env.status.unclassified': 'Referência não classificada — recomendação não verificada',
      'env.applicability.settings': 'Configurável via settings, sujeita a escopo e versão.',
      'env.applicability.subprocess': 'No settings.json afeta apenas subprocessos, não a interface do Claude.',
      'env.applicability.launch': 'Apenas no ambiente de inicialização; ignorada no settings.json.',
      'env.applicability.ignored': 'Gerenciada pela execução; ignorada no settings.json.',
      'env.applicability.unknown': 'Aplicabilidade e momento de leitura não foram verificados.',
      'env.guidance.nonempty': 'Qualquer valor não vazio, inclusive “0” e “false”, ativa a desativação. Não trate “0” como desligado.',
      'env.guidance.afk': 'Continuação automática vem desativada por padrão; “0” fecha imediatamente. O padrão de 60s existiu apenas nas versões v2.1.198–199.',
      'env.guidance.pwsh': 'Timeout do parser, não da execução do comando. O comportamento atual não é verificado.',
      'env.guidance.slowOperation': 'Logs de diagnóstico podem expor dados sensíveis. Nenhuma coleta é ativada automaticamente.',
      'env.guidance.cacheCost': 'Gravações com TTL de uma hora custam mais do que gravações com TTL de cinco minutos.',
      ...
      'models.discovery.status.noCreds': 'URL e Chave de API necessárias para descoberta de modelos.',
      'models.discovery.status.empty': 'Nenhuma URL e chave de API configuradas. A lista de modelos está vazia.',
      'models.discovery.badge.defaults': '{count} padrões',
      'models.discovery.badge.loaded': '{count} modelos',
      'models.discovery.badge.empty': '0 modelos',
      'models.discovery.hint': 'Consulta o endpoint /v1/models compatível com OpenAI para preencher os seletores de modelos.',
      'models.ai.generateBtn': 'IA',
      'models.ai.generateTitle': 'Gerar descrição com IA usando a API configurada',
      'models.ai.generating': 'Gerando descrição...',
      'models.ai.success': 'Descrição do modelo gerada com sucesso.',
      'models.ai.error': 'Falha ao gerar descrição: {error}',
      'models.ai.noModel': 'Insira um ID de Modelo antes de gerar uma descrição.',
      'models.ai.noApi': 'URL e chave da API são necessárias para gerar a descrição.',
      ...
      'models.field.streamFirstByteTimeout': 'Tempo Limite do Primeiro Byte de Streaming (ms)',
      'models.placeholder.streamFirstByteTimeout': 'ex: 60000',
      'models.hint.streamFirstByteTimeout': 'Prazo em ms para o primeiro byte de resposta em streaming (ex: 60000 para gateways).',
      'models.tiers.presets': 'Predefinições Rápidas:',
      'models.tiers.presetAlanwo': '+ Gateway Alanwo',
      ...
      'toggle.disableUnknownModelWindowEnforcement': 'Ignorar autocompactação para IDs de modelo desconhecidos do gateway',
      'toggle.attributionHeaderZero': 'Omitir cabeçalho de atribuição do prompt do sistema (cache do gateway)',
      ...
      'models.overrides.presetGateway': '+ Padrões do Gateway',
