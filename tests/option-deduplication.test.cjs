...
test('all ANTHROPIC_DEFAULT_*_MODEL_SUPPORTED_CAPABILITIES are removed from index.html and catalog dedicated keys', () => {
  const currentHtml = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
  const tiers = ['FABLE', 'OPUS', 'SONNET', 'HAIKU'];

  for (const tier of tiers) {
    const key = `ANTHROPIC_DEFAULT_${tier}_MODEL_SUPPORTED_CAPABILITIES`;
    assert.ok(
      !currentHtml.includes(`data-setting-path="env.${key}"`),
      `index.html must NOT contain env.${key}`
    );
    assert.ok(
      !currentHtml.includes(`id="env_${key}"`),
      `index.html must NOT contain env_${key} element ID`
    );
    assert.equal(
      catalog.isDedicatedEnvKey(key),
      false,
      `Catalog must not treat ${key} as a dedicated key`
    );
  }
});
